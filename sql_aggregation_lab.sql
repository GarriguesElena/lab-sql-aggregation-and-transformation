...
SELECT CONCAT(FLOOR(AVG(length) / 60), " Hour", " ", FLOOR(AVG(length) % 60), " Minutes") AS average_movie_duration FROM film;
...
-- 4. BONUS
SELECT CONCAT(first_name, " ", last_name) AS name, SUBSTRING(email, 1, 3) AS email FROM customer
...
SELECT COUNT(*) AS number_of_films, rating FROM film
GROUP BY rating;
...
SELECT last_name, COUNT(*) AS times_surname
FROM actor
GROUP BY last_name
HAVING times_surname = 1;